...
#pragma compile(FileDescription, [AD Tools v6b.0])
#pragma compile(ProductName, [AD Tools v6b.0])
...
#Region #VARIABLES Dynamiques
Global $__iAD_Debug = 0 ; Debug level: 0 = no debug info, 1 = Debug to console, 2 = Debug to MsgBox, 3 = Debug to File
...
Global $__oAD_Bind ; Ref to hold bind cache
...
; Global Const $ADS_FLAGTYPE_ENUM Enumeration
...
; GUIDs - LOWER CASE!
Global Const $USER_CHANGE_PASSWORD = "{ab721a53-1e2f-11d0-9819-00aa0040529b}" 			;  http://msdn.microsoft.com/en-us/library/cc223637(PROT.13).aspx
Global Const $SELF_MEMBERSHIP = "{bf9679c0-0de6-11d0-a285-00aa003049e2}" 				;  http://msdn.microsoft.com/en-us/library/cc223513(PROT.10).aspx
Global Const $ALLOWED_TO_AUTHENTICATE = "{68B1D179-0D15-4d4f-AB71-46152E79A7BC}" 		;  http://msdn.microsoft.com/en-us/library/ms684300(VS.85).aspx
Global Const $RECEIVE_AS = "{AB721A56-1E2f-11D0-9819-00AA0040529B}" 					;  http://msdn.microsoft.com/en-us/library/ms684402(VS.85).aspx
Global Const $SEND_AS = "{AB721A54-1E2f-11D0-9819-00AA0040529B}" 						;  http://msdn.microsoft.com/en-us/library/ms684406(VS.85).aspx
Global Const $USER_FORCE_CHANGE_PASSWORD = "{00299570-246D-11D0-A768-00AA006E0529}" 	;  http://msdn.microsoft.com/en-us/library/ms684414(VS.85).aspx
Global Const $USER_ACCOUNT_RESTRICTIONS = "{4C164200-20C0-11D0-A768-00AA006E0529}" 		;  http://msdn.microsoft.com/en-us/library/ms684412(VS.85).aspx
Global Const $VALIDATED_DNS_HOST_NAME = "{72E39547-7B18-11D1-ADEF-00C04FD8D5CD}" 		;  http://msdn.microsoft.com/en-us/library/ms684331(VS.85).aspx
Global Const $VALIDATED_SPN = "{F3A64788-5306-11D1-A9C5-0000F80367C1}" 					;  http://msdn.microsoft.com/en-us/library/ms684417(VS.85).aspx
#EndRegion
...
$defautdc = $defautdcinit
 ...
 Local $aPos = WinGetPos($hguiP)
   ...
   #Region GUI
			...
			GUICtrlSetTip($idcheckboxdriveremove, "Retirer un lecteur à l'Idrh 'Srce' " & @CRLF & @CRLF & "multiple idrh 'Srce', separateur [;] :  retirer le lecteur ayant la meme lettre aux users selectionnés [;]", "", 0, 1)
			GUICtrlSetState(-1, $gui_unchecked)
			    ...
			    Case $idok2
				  ...
				  ;$idgroup2 = GUICtrlRead($buttonidgroup2)
				   ...
				   If $idcheckboxdriveremove = 1 And $isdct = 1 And StringInStr($idrh1, ";") Then
					  ...
					  _GUICtrlRichEdit_SetFont($aff, 10)                   ;   set size
					  _GUICtrlRichEdit_SetCharAttributes($aff, '+bo')      ;   set bold
					  _GUICtrlRichEdit_AppendText($aff, @CRLF & "  Time: " & @HOUR & ":" & @MIN & ":" & @SEC & @CRLF & "Massive Users Drive [-] same Drive letter - (Retirer aux users separés par [;] un drive avec la meme lettre de lecteur)" & @CRLF);, 1)
					  _GUICtrlRichEdit_SetCharColor($aff, 0x00000000)      ;   set color black (BBGGRR format)
...
return $scomboreadGAUB
    ...
    Func massive_manualdrive() ;rajout 1 drive commun
...
Next ;$i
			...
			If $choixlecteur = "" or StringLen($choixlecteur)<4 Then
										MsgBox(0, "Warning !", "drive ! chemin non saisi / incomplet,vide... , abandon !", 7)
			;							Guidelete($hGUI)
										Return 0
			EndIf
			...
			If not StringInStr($choixlecteur,";\\") Then
										MsgBox(0, "Warning !", "drive ! chemin saisi manque:  [ ;\\ ] , abandon ..." & @CRLF & "saisie:  " & $choixlecteur, 7)
			;							Guidelete($hGUI)
										Return 0
			EndIf
...
;;
	...
	Func massive_drive() ;export des drives
		$defautdc = $defautdcinit
		If $isdct = 0 Then
			MsgBox(0, "Warning !", "Valable que pour le domaine DCT... ", 14)
			_GUICtrlRichEdit_AppendText($aff, @CRLF & "  Time: " & @HOUR & ":" & @MIN & ":" & @SEC & @CRLF & "Valable que pour le domaine DCT... " & @CRLF);, 1)
    ...
    Func massive_drive_remove() ;retire 1 drive ayant la meme lettre de lecteur
	...
	$choixlecteur= InputBox("Choix Drive [-] ?", "Donnez la lettre du lecteur à retirer (ex: N)", "", "")
    $choixlecteur= StringUpper($choixlecteur)

	if $choixlecteur="" or stringlen($choixlecteur)<>1 Then
	   MsgBox(0,"warning !","pas de lettre de lecteur, ou plus d'un caractere saisi, abandon !")
	   return 0
    EndIf


if StringIsAlpha ($choixlecteur) = 0 Then
	  MsgBox(0, "Warning !", "Pas de lettre de lecteur saisie ! " & @crlf "  Abandon ...", 7)
	  Return 0
EndIf


_GUICtrlRichEdit_AppendText($aff, @CRLF & "  Time: " & @HOUR & ":" & @MIN & ":" & @SEC & "    " & "Liste des users à retirer la lettre de lecteur [ " & $choixlecteur & " ]  :  [ " & $idrh1  & " ]" & @CRLF & @CRLF);, 1)
		 ...
		 If IsArray($idrh1) Then
			...
			For $z = 1 To $idrh1[0]
			   $choixlecteur2=""
				...
				If $userexist = 1 Then
				 ...
				 for $y= 0 to UBound($drivesidrh1)-1
			     $test=$drivesidrh1[$y]
				 $letter=StringUpper (StringLeft($test,1) )
				  if StringInStr($letter,$choixlecteur) Then
			        $choixlecteur2=$drivesidrh1[$y]
			        _ArrayDelete($drivesidrh1,$y)
				    ReDim $drivesidrh1
				    ExitLoop
			      EndIf
				 next ;$y
			...
			Global $ivalue = _ad_modifyattribute($idrh1[$z], "LaPoste00-NetworkDrive", $drivesidrh1, 2) ;,2) => update Drives, 3 provoquera des lecteurs tous dédoublés !
						...
						If $ivalue = 1 Then
						   $choixlecteur2=StringStripCR($choixlecteur2)
						   $choixlecteur2=StringStripWS($choixlecteur2,1+2)
						   if $choixlecteur2<>"" Then
	  _GUICtrlRichEdit_AppendText($aff,  "  Time: " & @HOUR & ":" & @MIN & ":" & @SEC & "    "  &  "  " & $idrh1[$z] & "  [-]  Drive:  " & $choixlecteur2  & "  : [OK]" & @CRLF);, 1)
						   Else
	  _GUICtrlRichEdit_AppendText($aff,  "  Time: " & @HOUR & ":" & @MIN & ":" & @SEC & "    "  &  "  " & $idrh1[$z] & "  [-]  Drive:  "  & "ne possède pas ce lecteur : [" & $choixlecteur & "]" & @CRLF);, 1)
						   endif
						ElseIf @error = 1 Then
									MsgBox(0, "info !", "[user Srce n'existe pas] : " & @CRLF & $idrh1[$z] , 14)
						Else
									MsgBox(0, "info !", "[Return error code " & @error & "] from Active Directory  :  " & $idrh1[$z] , 14)
						EndIf

			    Else ;userexist=0 ou vide...
				   if $idrh1[$z]<>"" then ; n'existe pas !
		_GUICtrlRichEdit_AppendText($aff,  "  Time: " & @HOUR & ":" & @MIN & ":" & @SEC & "    "  &  "  " & $idrh1[$z] & "  [-]  Drive:  n'existe  pas" & @CRLF);, 1)
	 Else ;user = champ vide ; ; ;
		 ; _GUICtrlRichEdit_AppendText($aff, @CRLF & "  Time: " & @HOUR & ":" & @MIN & ":" & @SEC & "    "  &  "user: " & $idrh1[$z] & "  [-]  Drive:  "  & "  :   ne possède pas ce lecteur  [" & $choixlecteur & "] !" & @CRLF);, 1)
		EndIf

				EndIf
			 Next ;$z

Else ;$idrh1 pas de [;] => pas une matrice
   MsgBox(0,"warning !","aucun separateur [;]")
   Return 0
EndIf
...
Func _AD_Open($sUserIdParam = "", $sPasswordParam = "", $sDNSDomainParam = "", $sHostServerParam = "", $sConfigurationParam = "", $iSecurity = 0)
	...
	Else
		...
		If @error Or Not IsObj($__oAD_RootDSE) Then Return SetError(4, @error, 0)
		$sAD_DNSDomain = $__oAD_RootDSE.Get("defaultNamingContext") ; Retrieve current AD domain name
		$sAD_HostServer = $__oAD_RootDSE.Get("dnsHostName") ; Retrieve name of connected DC
		$sAD_Configuration = $__oAD_RootDSE.Get("ConfigurationNamingContext") ; Retrieve Configuration naming context
		$__oAD_RootDSE = ObjGet("LDAP://" & $sAD_HostServer & "/RootDSE") ; guarantee persistant binding
	EndIf
	...
	If $sUserIdParam <> "" Then
		...
		If @error Or Not IsObj($__oAD_Bind) Then ; login error occurred - get extended information
			...
			If Int($sOSVersion[1]) >= 6 Then ; Delivers detailed error information if debugging is activated (>Vista)
...
Func _AD_ErrorNotify($iDebug, $sDebugFile = "")
	...
	Switch $iDebug
		...
		Case Else
			...
			; A COM error will be init only if does not exist
			...
			ElseIf ObjEvent("AutoIt.Error") = "__AD_ErrorHandler" Then
				Return SetError(0, 0, 1) ; COM error handler already set by a call to this Func
			Else
...
Func _AD_IsMemberOf($sGroup, $sObject = @UserName, $bIncludePrimaryGroup = False, $bRecursive = False, $iDepth = 10)
	...
	If $iResult = 0 And $bRecursive = True Then
		...
		Next
	EndIf
	; Check Primary Group if $sObject not member of the specified group and the flag is set
...
Func _AD_GetUserPrimaryGroup($sUser = @UserName)
	...
	$__oAD_Command.CommandText = "<LDAP://" & $sAD_HostServer & "/" & $sAD_DNSDomain & ">;(" & $sProperty & "=" & $sUser & ");ADsPath;subtree"
	Local $oRecordSet = $__oAD_Command.Execute ; Retrieve the FQDN for the logged user
	Local $sLDAPEntry = $oRecordSet.fields(0).value
	Local $oObject = __AD_ObjGet($sLDAPEntry) ; Retrieve the COM Object for the logged user
...
Func _AD_ListDomainControllers($bListRO = False, $bListGC = False)
	...
	; Controller. The parent of each Domain Controller is a "Server"
...
Func _AD_GetLastLoginDate($sObject = @UserName, $sSite = "", $aDCList = "")
	...
	For $iCount1 = 1 To $aDCList[0][0]
		...
		; -2147352567 or 0x80020009 is returned when the service not operational
		...
		Else
			...
			; -2147352567 or 0x80020009 is returned when the attribute "LastLogin" Not defined on DC
			If @error = -2147352567 Then $iError2 += 1
			$oObject.PurgePropertyList
		EndIf
	Next
	_ArraySort($aResult, 1, 1)
	; If error count == the number of DCs then the user has never logged in
...
Func _AD_IsPasswordExpired($sAccount = @UserName)
	...
	If $aTemp[13] <= _NowCalc() Then Return 1 ; Compare to calculated expire date/time
	Return
...
Func _AD_GetPasswordExpired($sRoot = "", $bNeverChanged = False, $iPasswordAge = 0, $bComputer = False)
	...
	If $iPasswordAge <> 0 Then
		$sDTExpire = _DateAdd("D", $iPasswordAge * -1, $sDTExpire) ; substract max password age
	Else
		$sDTExpire = _DateAdd("D", $aTemp[1] * -1, $sDTExpire) ; substract max password age
	EndIf
...
Func _AD_GetObjectProperties($vObject = @UserName, $sProperties = "", $bSort = True)
	...
	If Not IsObj($vObject) Then
		If StringLeft($vObject, 7) <> "LDAP://" Then ; No ADsPath
			...
			Local $oRecordSet = $__oAD_Command.Execute ; Retrieve ADsPath for the object
			If @error Or Not IsObj($oRecordSet) Then Return SetError(3, @error, "")
			$vObject = $oRecordSet.fields(0).Value
		EndIf
		$oObject = __AD_ObjGet($vObject) ; Retrieve COM Object
	Else
		$oObject = $vObject
	EndIf
	If $sProperties = "" Then
	...
	If $bSort And $iPropertyRecord > 1 Then _ArraySort($aObjectProperties, 0, 1) ; Only sort if flag set and array contains > 1 records
...
Func _AD_ModifyAttribute($sObject, $sAttribute, $sValue = "", $iOption = 1)
	...
	If StringMid($sObject, 3, 1) = "=" Then $sProperty = "distinguishedName" ; FQDN provided
	$__oAD_Command.CommandText = "<LDAP://" & $sAD_HostServer & "/" & $sAD_DNSDomain & ">;(" & $sProperty & "=" & $sObject & ");ADsPath;subtree"
	Local $oRecordSet = $__oAD_Command.Execute ; Retrieve ADsPath for object
	Local $sLDAPEntry = $oRecordSet.fields(0).Value
	Local $oObject = __AD_ObjGet($sLDAPEntry) ; Retrieve COM Object
	$oObject.GetInfo
	If $sValue = "" Then
		$oObject.PutEx(1, $sAttribute, 0) ; CLEAR: remove all the property value(s) from the object
	ElseIf $iOption = 3 Then
		$oObject.PutEx(3, $sAttribute, $sValue) ; 3 == APPEND: append the specified value(s) to the existing values(s)
	ElseIf IsArray($sValue) Then
		$oObject.PutEx(2, $sAttribute, $sValue) ; 2 == UPDATE: replace the current value(s) with the specified value(s)
	Else
...
Func _AD_RenameObject($sObject, $sCN)
	...
	If StringMid($sObject, 3, 1) <> "=" Then $sObject = _AD_SamAccountNameToFQDN($sObject) ; sAMAccountName provided
	Local $oObject = __AD_ObjGet("LDAP://" & $sAD_HostServer & "/" & $sObject)
	Local $oOU = __AD_ObjGet($oObject.Parent) ; Get the object OU/CN where the object resides
	$sCN = "CN=" & _AD_FixSpecialChars($sCN) ; escape all specs chars
...
Func _AD_MoveObject($sOU, $sObject, $sCN = "")
	...
	Local $oOU = __AD_ObjGet("LDAP://" & $sAD_HostServer & "/" & $sOU) ; Pointer to destination container
	$oOU.MoveHere("LDAP://" & $sAD_HostServer & "/" & $sObject, $sCN)
	If @error Then Return SetError(@error, 0, 0)
	Return 1
...
Func _AD_EnablePasswordChange($sObject)
	...
	; Get the dependant accounts for SELF and EVERYONE
...
Func _AD_GetPasswordInfo($sObject = @UserName)
	...
	Else
		...
		; Have to convert to SystemTime because of  _Date_Time_FileTimeToStr
...
Func _AD_SetPasswordExpire($sUser, $iFlag = 0)
	...
	If StringMid($sUser, 3, 1) = "=" Then $sProperty = "distinguishedName" ; FQDN provided
	$__oAD_Command.CommandText = "<LDAP://" & $sAD_HostServer & "/" & $sAD_DNSDomain & ">;(" & $sProperty & "=" & $sUser & ");ADsPath;subtree"
	Local $oRecordSet = $__oAD_Command.Execute ; Retrieve ADsPath of object
	Local $sLDAPEntry = $oRecordSet.fields(0).Value
	Local $oObject = __AD_ObjGet($sLDAPEntry) ; Retrieve the COM Object
...
Func _AD_IsAccountExpired($sObject = @UserName)
	...
	$__oAD_Command.CommandText = "<LDAP://" & $sAD_HostServer & "/" & $sAD_DNSDomain & ">;(" & $sProperty & "=" & $sObject & ");ADsPath;subtree"
	Local $oRecordSet = $__oAD_Command.Execute ; Retrieve the ADsPath of object
	Local $sLDAPEntry = $oRecordSet.fields(0).Value
	Local $oObject = __AD_ObjGet($sLDAPEntry) ; Retrieve the COM Object
...
Func _AD_FixSpecialChars($sText, $iOption = 0, $sEscapeChar = '"\/#,+<>;=')
	...
	If $iOption = 0 Then
		; "?<!" means: Look behind assertion (negative)
	...
	ElseIf $iOption = 1 Then
		...
		If StringInStr($sEscapeChar, '\') Then $sText = StringReplace($sText, '\\', '\')
	Else
		; escape-specs-chars/#entry1136636
...
Func _AD_GetLastADSIError()
	...
	$aLastError[2] = DllStructGetData($ED, 1) ; Unicode string describes the error
	$aLastError[3] = DllStructGetData($PN, 1) ; name of provider that raised error
...
Func _AD_VersionInfo()
	...
	Local $aOL_VersionInfo[9] = [8, "V", 1, 4, 10.0, "20181019", "Autoit 3.3.12", "Nicolas RISTOVSKI 2018/2019", ""]
